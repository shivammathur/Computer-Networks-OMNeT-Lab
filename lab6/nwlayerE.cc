...
Define_Module(NwlayerE);

void NwlayerE::initialize()
{
    nid=par("nid");
    src=par("src");
    dest=par("dest");
    numSent=0;
    numRec=0;
    table[1]=make_pair(6,3);
    table[2]=make_pair(3,0);
    table[3]=make_pair(4,0);
    table[4]=make_pair(9,3);
    table[5]=make_pair(0,3);
...
void NwlayerE::updateDisplay()
{
    char buf[40];
    sprintf(buf, "rcvd: %d sent: %d", numRec, numSent);
    getDisplayString().setTagArg("t",0,buf);
}
void NwlayerE::finish(){
