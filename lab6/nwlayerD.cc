...
#include "A_PDU_m.h"
#include "NW_PDU_m.h"
Define_Module(NwlayerD);

void NwlayerD::initialize()
{
    nid=par("nid");
    src=par("src");
    dest=par("dest");
    numSent=0;
    numRec=0;
    table[1]=make_pair(3,0);
    table[2]=make_pair(8,1);
    table[3]=make_pair(5,1);
    table[4]=make_pair(0,0);
    table[5]=make_pair(9,1);
...
void NwlayerD::handleMessage(cMessage *msg)
{
    numRec++;
        if (ev.isGUI())
            updateDisplay();
...
void NwlayerD::updateDisplay()
{
    char buf[40];
    sprintf(buf, "rcvd: %d sent: %d", numRec, numSent);
    getDisplayString().setTagArg("t",0,buf);
}
void NwlayerD::finish(){
